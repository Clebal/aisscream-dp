...
public interface RendezvousRepository extends JpaRepository<Rendezvous, Integer> {
	...
	@Query("select count(distinct r.rendezvous) from Request r join r.service.categories c where c.id=?1")
	Integer countFindByCategoryId(int categoryId);

	@Query("select distinct(r.rendezvous) from Request r join r.service.categories c where c.id=?1 and r.rendezvous.adultOnly=false")
	Collection<Rendezvous> findByCategoryIdAllPublics(int categoryId);

	@Query("select count(distinct r.rendezvous) from Request r join r.service.categories c where c.id=?1 and r.rendezvous.adultOnly=false")
	Integer countFindByCategoryIdAllPublics(int categoryId);
