...
public interface NewspaperRepository extends JpaRepository<Newspaper, Integer> {
	...
	@Query("select n from Newspaper n where n.publicationDate >= CURRENT_DATE && n.isPrivate=false && n.isPublished=true ")
	...
	@Query(value = "select n.id from Newspaper n where n.id NOT IN (select (s.newspaper_id) from Subscription s where s.customer_id=?1) and n.publicationDate <= CURRENT_DATE && n.isPrivate=true", nativeQuery = true)
