...
public interface NewspaperRepository extends JpaRepository<Newspaper, Integer> {
	...
	@Query(value = "select count(n.id) from Newspaper n where n.id NOT IN (select (s.newspaper_id) from Subscription s where s.customer_id=?1) and n.publicationDate <= CURRENT_DATE and n.isPrivate=true", nativeQuery = true)
